...
queens n = queens' [] 1 1 where
    ...
    queens' placed col row
        ...
        | row > n = []
        | col == n =
            if canPut placed col row then (placed ++ [row]):nextRow else nextRow
        | otherwise =
            if canPut placed col row then nextRow ++ nextCol else nextRow
        where
...
infiniteRoseTree = RNode 1 (repeat infiniteRoseTree)
