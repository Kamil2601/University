...
class Agent:
    ...
    def agent_move(self,board, player):
        ...
        board.do_move(best_move,player)
    ...
    def heuristic(self,board,player):
        return 10000*self.pieces_balance(board)+self.cave(board)+5*self.dist(board)
...
for i in range(1):
    player = 1
    B = Board()
    while B.game_over(player)==False:
        if player==ag:
            A.agent_move(B,player)
        else:
            B.random_agent(player)
        player = 1-player
        B.draw()
        print()
    if B.check_result(player)==ag:
        wins+=1
    ag = 1-ag
