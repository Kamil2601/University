...
class Agent:
    ...
    def sum_distance(self,board):
        l_w = len(board.pieces[0])
        l_b = len(board.pieces[1])
        ...
        white_p = copy.deepcopy(board.pieces[0])

        for i in range(m):
            fig = max(white_p.items(), key=operator.itemgetter(1))[0]
            white_d+=abs(board.pos[0][fig][0]-8)+abs(board.pos[0][fig][1]-3)
            white_p.pop(fig)

        black_d = 0

        black_p = copy.deepcopy(board.pieces[1])

        for i in range(m):
            fig = max(black_p.items(), key=operator.itemgetter(1))[0]
            black_d+=abs(board.pos[1][fig][0]-0)+abs(board.pos[1][fig][1]-3)
            black_p.pop(fig)

        return black_d-white_d
    ...
    def near_stronger(self,board,player):
        op = 1-player
        ...
        for piece in board.pos[player]:
            y = board.pos[player][piece][0]
            x = board.pos[player][piece][1]
            if board.board[y][x][0]=='.':
                if y>0:
                    p = board.board[y-1][x][1]
                    q = board.board[y-1][x][0]
                    if q!='-':
                        if p in board.pieces[op]:
                            if board.pieces[op][p]>=board.pieces[player][piece]:
                                res+=1
                                continue
                    else:
                        rat = False
                        for i in range(y-1,y-4,-1):
                            if board.board[i][x][1] in board.pieces[player]:
                                rat = True
                                break
                        if rat==False:
                            ep = board.board[y-4][x][0]
                            if ep in board.pieces[op] and (ep==l or ep==t):
                                if board.pieces[op][ep]>=board.pieces[player][piece]:
                                    res+=1
                                    continue
                if y<8:
                    p = board.board[y+1][x][1]
                        ...
                        if rat==False:
                            ...
                            if ep in board.pieces[op] and (ep==l or ep==t):
                                if board.pieces[op][ep]>=board.pieces[player][piece]:
                                    ...
                                    continue
                if x>0:
                    p = board.board[y][x-1][1]
                    q = board.board[y][x-1][0]
                    if q!='-':
                        if p in board.pieces[op]:
                            if board.pieces[op][p]>=board.pieces[player][piece]:
                                res+=1
                        ...
                        for i in range(x-1,x-3,-1):
                            if board.board[y][i][1] in board.pieces[player]:
                                rat = True
                                break
                        if rat==False:
                            ep = board.board[y][x-3][0]
                            if ep in board.pieces[op] and (ep==l or ep==t):
                                if board.pieces[op][ep]>=board.pieces[player][piece]:
                                    res+=1
                                    continue
    ...
    def heuristic(self,board,player):
        return 10*self.pieces_balance(board)+self.sum_distance(board)+self.cave(board)
