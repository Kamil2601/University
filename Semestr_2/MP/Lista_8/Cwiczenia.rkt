...
(define (eval-cadar e)
  (define (iter e l)
    (cond [(eq? (car l) 'r) e]
          [(eq? (car l) 'a) (iter (car e) (cdr l))]
          [(eq? (car l) 'd) (iter (cdr e) (cdr l))]))
  (iter (cdr e) (cdr (split-symbol (car e)))))
...
(define xd '(lambda (x) (+ x 1)))
(map (lambda (x) (apply-closure (eval-env xd '()) (list x))) '(1 2 3))

(define (map-eval e env)
  (map (lambda (x) (apply-closure (eval-env (second e) env) (list x))) (third e)))

(map-eval '(map (lambda (x) (+ x 1)) (1 2 3)) '())

(eval-env '(map (lambda (x) (+ x 1)) (1 2 3)) '())
