...
;;Pomocnicza procedura zamieniająca zmienną na wywołanie procedury ('x na '(x))

(define (var->proc s e)
  (cond [(list? e) (map (lambda (x) (var->proc s x)) e)]
        [(eq? s e) (list e)]
        [else e]))

;;Główna procedura zamieniająca lazy-let na let z lambdą w definicji
;;i wywołaniem procedury zamiast zmiennej

(define (change-let e)
  (let ((new-def (list (lazy-let-def-var (lazy-let-def e))
                       (list 'lambda '() (lazy-let-def-expr (lazy-let-def e)))))
        (new-expr (var->proc (lazy-let-def-var (lazy-let-def e)) (lazy-let-expr e))))
    (let-cons new-def new-expr)))
...
(eval '(lazy-let (x (/ 1 0)) 1))
(eval '(lazy-let (x (+ 3 4)) (+ x x)))
(eval '(lazy-let (x (lazy-let (y (lazy-let (z 3) (+ z 1))) (+ y 1))) (+ x 1)))
(eval '(let (x 3) (lazy-let (y (/ 1 0)) x) ))
(eval '(+ 1 (lazy-let (y (/ 1 0)) 1)))
(eval '(lazy-let (x (/ 1 0)) (+ (lazy-let (y (+ x 1)) 1) (lazy-let (z (+ y 1)) 1))))
(eval '((lambda (x) (lazy-let (y 1) (+ x y))) 1))
(eval '((lambda (x) (lazy-let (y (/ 1 0)) (+ x 10))) 1))
(eval '(lazy-let (x 0) x))
