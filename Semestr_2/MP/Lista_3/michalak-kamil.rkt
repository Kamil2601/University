...
(define (solve-ax+by=1 a b)
  (define (div k l)
    (/ (- k (modulo k l)) l))
  (define (iter s t r old_s old_t old_r)
    (if (= r 0)
        (cons old_s old_t)
        (let ((x (div old_r r)))
          (iter (- old_s (* x s))
...
(define (encrypt-and-sign message recipient-public-key sender-private-key)
  (define encrypted-message (RSA-encrypt message recipient-public-key))
  (define sign (RSA-transform message sender-private-key))
  (make-signed-message encrypted-message sign))

(define (authenticate-and-decrypt cyphertext recipient-private-key sender-public-key)
  (define message (RSA-decrypt (message cyphertext) recipient-private-key))
  (define hashed-message (compress (string->intlist (message cyphertext)) ))
  (define transformed-sign (RSA-transform  (signature cyphertext) sender-public-key))
  (if (= hashed-message transformed-sign)
      message
      (error "Podpis sie nie zgadza")))
